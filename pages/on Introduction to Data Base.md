	...
	- 范式
		...
		- 第三范式
			...
			- 定理
				- 内容
					- $R(U)$不满足第三范式
					- 当前仅当有单个非主属性**完全依赖**于其他非关键字属性集
					- 即$X\overset{f}{\to} Y$且$X$不是关键字
					- 也就是说，$X$还不是关键字，$Y$就被它完全决定了
				- 证明
					- 必要性
						- 若$X\overset{f}{\to} Y$且$X$不是关键字
						- 1. $X$是某个关键字真子集，则$R\notin 2NF$
						- 2. $X$不是某个关键字的真子集，也根本不是关键字
						      则必然存在关键字“经由决定$X$，从而决定$Y$”的情况；因为$Y$完全依赖于$X$。
					- 充分性
						- 若$R(U)\notin 3NF$，则有某个非主属性$t$传递依赖于关键字$X$。
						- 则这个非主属性必须先依赖于某个非关键字$Y$，且$t\notin Y$。
						- $t$会完全依赖于$Y$的哪个子集呢？如果这个子集$Z$是非关键字，则结束证明。
						- 假设子集$Z$**总**是关键字，对于所有$Y\to t$的$Y$来说。（否则仍可用上一步结束证明）
						- 则：$Y\to t\implies Z\overset{f}{\to} t\text{ $Z$为关键字}$
						-
						-
