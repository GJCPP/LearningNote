...
- 关系数据结构
	...
	- 关键字
		- 定义：能**唯一极小**标识元组的属性集
			- 唯一：当这个集合中的属性确定后，所有属性都确定了
			- 极小：去掉这个集合中任一个属性，它不再能决定所有属性
		- 并非组成成分，只是理论上存在之物。
...
- 规范化设计理论
	...
	- Armstrong公理系统
		...
		- 函数依赖集等价
			- 即$F_1^+=F_2^+$
			- 最小函数依赖集：不存在部分依赖，且去掉其中任一依赖，不再等价。（其实应该叫极小）
		- 属性集闭包
	...
	- 范式
		- 所有范式的定义
			- 第一范式：不说了
			- 第二范式：第一范式+所有非主属性完全依赖于关键字
			- 第三范式：第二范式+所有非主属性不传递依赖于关键字
			- BC范式：第三范式+所有所有主属性不完全依赖于关键字的真子集
			- 第四范式：第三范式+任何非平凡的多值依赖$X\to\to Y$均满足$X$含关键字
		- 第一范式
			- 定义
		...
		- 第三范式
			...
			- 定理
				- 内容
					...
					- 且这个非关键字属性集中不含这个非主属性自己。
					- **完全依赖**是必要的，否则可以造一个关键字超集的依赖出来。
				- 证明
					- 充分性
						- 若其不满足第二范式，则有非主属性部分依赖于关键字，易证。
						- 若其满足第二范式，则有非主属性传递依赖于某个关键字
						- 不妨记为$X\overset{f}{\to} Y\to t$，且$Y\not\to X$（传递依赖要求）
						- 则$Y$一定是非关键字。
						- 在$Y$中取子集使得$t$完全依赖于该子集；证毕。
					- 必要性
						- 若有非主属性$t$完全依赖于（不含$t$的）非关键字属性集$Y$，取最小的这样的$Y$。
						- 假设满足第三范式；取任一关键字$X$，有$X\to Y$.
						- 则$X\to Y\overset{f}{\to} t$.
						- 同时，由于$Y$是最小的这样的非关键字，$Y\not\to X$. （否则$Y$是关键字）
						- 因此$t$传递依赖与关键字$X$。不是第三范式。
		...
		- 巴斯-科德范式 (Boyce-Codd Norm Form)
			- 定义
				...
				- 另一个定义是：在第三范式上，任何主属性不能非平凡依赖于关键字子集
			- 含义
				- 只有关键字能决定非自己的某些部分
			- $R(U)\in BCNF\implies R(U)\in 3NF$.
				- 证明
					- 若有非主属性$t$完全依赖于某个不含自己的非关键字属性$Y$（反驳了判定第三范式的定理）
					- 则$Y\to t(t\notin Y)$意味着$Y$含关键字。
					- "$t$完全依赖于一个关键字的超集"是假命题。
					- 证毕。
			- What to do: 满足3NF但不满足BCNF
				- BC范式对第三范式的加强在于，要求主属性不能非平凡地依赖于关键字子集
				- 例子如，若$A\to C$，$AB\to D$，$CD\to AB$；的关键字是$AB$与$CD$
				- 这是满足第二、三范式的：没有非主属性
				- 但是由于$A\to C$，主属性依赖于关键字真子集
				- 可以把它拆为关系$R(A,C)\cup R(AB, D)$
		- 第四范式
			- 定义
	...
	- 模式分解
		...
		- 分解目标
			- 无损联接性
				- 若$\rho=\{R_1, ..., R_k\}$是对$R(U)$的一个分解，$F$为$R(u)$函数依赖集合
				- 若$R(U)$中满足$F$的关系实例$r$均有$r=\pi_{R_1}(r)\Join ... \Join \pi_{R_k}(r)$
				- 则称分解$\rho$相对于$F$是无损联接分解。
				- **这是分解的正确性，一定要被满足**
			- 依赖保持性
				- 记“$F$在属性集$Z$上投影”为$\pi_Z(F)=\{X\to Y\in F^+\ |\ X\cup Y\subseteq Z\}$
				- 若分解$\rho=\{R_1, ..., R_k\}$满足$F^+=\pi_{R_1}(F)\cup ... \cup \pi_{R_k}(F)$
				- 则称其具有依赖保持性。
				- 判定定理
					- 若将$R$分解为$\rho=(R_1,R_2)$，$F$为$R$的函数依赖集合
					- $\rho$具有依赖保持性，当且仅当以下一者得到满足
		...
		- 分解算法（最高可保证到第三范式）
			...
			- 3. 对每个$X\to Y\in F$
				...
				- 则将$X\cup Y$加入$S$中
				- （此步为依赖保持）
			- 4. 若找不到$R$的候选关键字$K$，其含于某个$Head(Z)$，$Z\in S$
				- 则任取一个候选关键字$K$加入$S$中
				- （此步为无损联接：第三步做完了所有依赖后，再塞入关键字，就能保证决定所有子关系了）
		-